...
Korquad는 한 지문에 여러개의 질문 / 답변이 매칭되어있는 데이터셋이다. 주어진 자료를 적극적으로 활용하여, 답변도 학습에 활용하면 어떨까? 다음과 같은 데이터셋을 고안해보았다.
...
이렇게 구성하면 같은 입력에 대해서는 똑같은 출력이 나오는 것을 보장할 수 있고, 또한 명시적으로 정답을 먼저 알려주기 때문에 더 명확한 질문을 생성할 수도 있다는 기대도 들었다.

![image](https://user-images.githubusercontent.com/35295182/176619916-5e86f6a9-5e7b-4a94-89ea-4ddf3d7ec20a.png)

기대한 것과 동일한 결과물이 나와주었다. 한 지문에서도 서로 다른 정답을 알려주었을 때 올바른 질문을 적절하게 생성하는 모습을 보여주었다. 만약 지문에서 정답 후보를 추출해내는 모델을 개발할 수 있다면, 정답 생성 모델과 질문 생성 모델을 파이프라인으로 이어서 구현하면 질문생성 태스크를 잘 수행해낼 수 있겠다는 생각이 들었다.

연구 과정에서 얻어낸 몇가지 재밌는 결과가 있다.
...
정답을 알려주지 않으면 지문 전체를 요약하는 질문을 생성한다.
